...
import pickle 
dimension_ = 128
load_ = "models/128_test_isolet_3layers_step4.pt"
load_True = True
test_hdc = False
save_ = "64_test_isolet_3layers_step4.pt"
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc0 = nn.Linear(1851, 2 * dimension_, bias=False)
        self.Bn0 = nn.BatchNorm1d(2 * dimension_)

        self.fc0_ = nn.Linear(2 * dimension_, dimension_, bias=False)
        ...
        self.fc1 = nn.Linear(dimension_, 26, bias=False)
        self.relu = nn.ReLU()
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        onehot = torch.nn.functional.one_hot(target, 26)
        optimizer.zero_grad()
        output = model(data)
...
def test_represent(model, device, train_loader,test_loader,threshold_):
    ...
    represent_ = torch.zeros((26,dimension_)).cuda()
    with torch.no_grad():
...
def main():
    ...
    x_train, y_train = quantize(torch.tensor(x_train)).unsqueeze(1).to(torch.float32), torch.tensor(y_train).long()
    x_test, y_test = quantize(torch.tensor(x_test)).unsqueeze(1).to(torch.float32), torch.tensor(y_test).long()
    # x_train, y_train = torch.tensor(x_train).unsqueeze(1).to(torch.float32), torch.tensor(y_train).long()
    # x_test, y_test = torch.tensor(x_test).unsqueeze(1).to(torch.float32), torch.tensor(y_test).long()
    ...
    if test_hdc:
        repre = test_represent(model, device, train_loader,test_loader,55)
        repre = test_represent(model, device, train_loader,test_loader,60)
        repre = test_represent(model, device, train_loader,test_loader,100)
        repre = test_represent(model, device, train_loader,test_loader,200)
        repre = test_represent(model, device, train_loader,test_loader,300)
